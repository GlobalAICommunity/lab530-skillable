...
matters because guide missions should be solved with retrieval from the
Driebergen content instead of hard-coded answers or model guesses.
...
7. For **Description**, enter **A Driebergen city guide with neighborhoods, history, food, culture, transit, and local knowledge**.
...
2. For **Description**, enter **Driebergen city guide knowledge base for grounded workshop guide answers**.
...
```text
What is the name of the traditional forest tearoom in Heidestein?
```

The answer should mention **Theehuis Heidestein**.
...
```python-notype
"""City guide specialist agent for Driebergen knowledge-base questions."""
...
def build_city_guide_agent():
	...
	guide_agent = Agent(
		client=client,
		name="Driebergen City Guide Agent",
		instructions=(
			"Answer Driebergen city guide questions using the provided search context. "
...
def build_city_guide_tool():
	...
	guide_tool = guide_agent.as_tool(
		name="ask_city_guide",
		description=(
			"Ask the Driebergen city guide knowledge base a question."
		),
		arg_name="question",
...
async def main() -> None:
	...
	question = "What is the name of the traditional forest tearoom in Heidestein?"
	guide_agent, search_context_provider = build_city_guide_agent()
...
> knowledge base. The answer should mention **Theehuis Heidestein**.
